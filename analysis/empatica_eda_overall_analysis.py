...
baseline_folder = "analysis/empatica_eda_baseline"
task_folder = "analysis/empatica_eda_task"
...
for patient_file in os.listdir(baseline_folder):
    if patient_file.endswith(".csv"):
        ...
        baseline_eda_all.append(baseline_data['eda'])
        task_eda_all.append(task_data['eda'])

        # Clean up outliers in baseline and task data
        baseline_clean = remove_outliers(baseline_data['eda'])
        task_clean = remove_outliers(task_data['eda'])
        ...
        axs[0].set_title(f'Patient {len(baseline_means)-1}: Electrodermal Activity Distribution (Outliers Removed)')
        axs[0].set_ylabel('Electrodermal Activity')
        ...
        axs[1].set_title(f'Patient {len(baseline_means)-1}: Electrodermal Activity Distribution (Outliers Removed)')
        axs[1].set_xlabel('Electrodermal Activity')
