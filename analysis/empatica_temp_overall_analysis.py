...
baseline_folder = "analysis\empatica_temp_baseline"
task_folder = "analysis\empatica_temp_task"
...
for patient_file in os.listdir(baseline_folder):
    if patient_file.endswith(".csv"):
        ...
        baseline_temp_all.append(baseline_data['temp'])
        task_temp_all.append(task_data['temp'])

        # Clean up outliers in baseline and task data
        baseline_clean = remove_outliers(baseline_data['temp'])
        task_clean = remove_outliers(task_data['temp'])
        ...
        axs[0].set_title(f'Patient {len(baseline_means)-1}: Body Surface Temperature Distribution (Outliers Removed)')
        axs[0].set_ylabel('Body Surface Temperature')
        ...
        axs[1].set_title(f'Patient {len(baseline_means)-1}: Body Surface Temperature Distribution (Outliers Removed)')
        axs[1].set_xlabel('Body Surface Temperature')
